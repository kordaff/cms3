...
my $dbh =
  DBI->connect( "dbi:Pg:dbname=userconfig", 'userconfig', '',
    { AutoCommit => 1 } )
  or die $!;
my ( @row, $sth, $query );
...
my $domain = 'vitriol.perl-user.com';
...
sub create_table_pages {
    ...
    $sth = $dbh->do(
"CREATE TABLE pages (body text,url text,domain text,creation_time integer,username text,useruuid uuid,remote_addr text,active boolean,version_num integer,id SERIAL PRIMARY KEY);"
    ) or die $!;
...
sub create_table_comments {
    $sth = $dbh->do(
"CREATE TABLE comments (body text,url text,domain text,comment_time integer,remote_addr text,username text,useruuid uuid,id SERIAL PRIMARY KEY); "
    ) or die $!;
}

sub create_table_notes {
    $sth = $dbh->do(
"CREATE TABLE notes (body text,url text,domain text,note_time integer,remote_addr text,username text,useruuid uuid,id SERIAL PRIMARY KEY); "
    ) or die $!;
}

sub create_table_access_log {
    $sth = $dbh->do(
"CREATE TABLE access_log (url text, domain text,method text,access_time timestamp without time zone,remote_ip text,refer text,user_agent text,created timestamp without time zone,id SERIAL PRIMARY KEY); "
    ) or die $!;
}

sub create_table_user_data {
    $sth = $dbh->do(
"CREATE TABLE user_data (username text,useruuid uuid,email text,user_created integer,id SERIAL PRIMARY KEY,password TEXT,delete_password TEXT); "
    ) or die $!;
...
sub load_api_form_pages {
    ...
    $body =~ s/__DOMAIN__/$domain/;
    $sth = $dbh->do(
"INSERT INTO pages (body,url,domain,creation_time,username,useruuid,remote_addr,active,version_num) values ('$body','/api/add_page','$domain','$t','admin','$uuid','72.201.204.99',true,1)"
    );
    ...
    $body =~ s/__DOMAIN__/$domain/;
    $sth = $dbh->do( "INSERT INTO pages (body,url,domain,creation_time,username,useruuid,remote_addr,active,version_num) values ('$body','/api/login','$domain','$t','admin','$uuid','72.201.204.99',true,1)");
...
#    }
#    $body =~ s/__DOMAIN__/$domain/;
#    $sth = $dbh->do( "INSERT INTO pages (body,url,domain,creation_time,username,useruuid,remote_addr, active,version_num) values ('$body','/api/register','$domain','$t','admin','$uuid','72.201.204.99',true,1)");
...
sub create_table_sessions {
    $sth = $dbh->do(
"CREATE TABLE sessions (sessionuuid uuid,useruuid uuid,remote_addr text,last_access integer,last_login integer,id SERIAL PRIMARY KEY);"
    ) or die $!;
...
sub salt {
    ...
    return Crypt::Eksblowfish::Bcrypt::en_base64( Crypt::Random::Source::get_strong(12));
}
